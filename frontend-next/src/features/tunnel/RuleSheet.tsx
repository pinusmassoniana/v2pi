 ...
 */
export function RuleSheet({ row, index, count, onUpdate, onMove, onRemove, onClose }: RuleSheetProps) {
  const n = index + 1;
      ...
      <SheetContent title={`Rule ${n} of ${count}`} className="max-md:max-h-[92dvh]">
        <div className="flex flex-col gap-3.5">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Button size="sm" disabled={index === 0} onClick={() => onMove(row.key, -1)}><ArrowUp size={14} aria-hidden />Move up</Button>
            <Button size="sm" disabled={index === count - 1} onClick={() => onMove(row.key, 1)}><ArrowDown size={14} aria-hidden />Move down</Button>
            <Button
              size="sm"
              variant="danger"
              onClick={() => {
