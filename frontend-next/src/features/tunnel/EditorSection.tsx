...
export function EditorSection({ title, note, collapsible, open, onToggle, summary, children }: EditorSectionProps) {
  const id = useId();
  ...
  return (
    <fieldset className="m-0 min-w-0 border-0 border-t border-line p-0 first:border-t-0">
      <legend className="float-left w-full py-2.5 min-[1180px]:sticky min-[1180px]:top-0 min-[1180px]:z-10 min-[1180px]:bg-solid">
        {collapsible ? (
          <button
            type="button"
            aria-expanded={open}
            aria-controls={id}
            onClick={onToggle}
            className="flex min-h-9 w-full items-center gap-2 text-left focus-visible:outline-2 focus-visible:outline-g2"
          >
            {heading}
