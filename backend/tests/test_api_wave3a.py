...
def test_setup_creates_credential_and_authenticates(settings, stub_xray):
    ...
    r = c.post("/api/setup", json={"username": "admin", "password": "s3cret"})
    ...
    assert c.post("/api/setup", json={"username": "x", "password": "y"}).status_code == 409
...
def test_login_with_created_credential(settings, stub_xray):
    ...
    assert c.post("/api/login", json={"username": "nope", "password": "s3cret"}).status_code == 401
    assert c.post("/api/login", json={"username": "admin", "password": "s3cret"}).status_code == 200
    assert c.get("/api/csrf").json()["csrf"]


def test_password_change(settings, stub_xray):
    c = _client(settings, stub_xray)
    c.post("/api/setup", json={"username": "admin", "password": "old"})
    h = {"X-CSRF-Token": c.get("/api/csrf").json()["csrf"]}
    # csrf required
    assert c.post("/api/password", json={"current_password": "old", "new_password": "new"}).status_code == 403
    # wrong current rejected
    assert c.post("/api/password", json={"current_password": "nope", "new_password": "new"},
                  headers=h).status_code == 403
    # correct current rotates the hash
    assert c.post("/api/password", json={"current_password": "old", "new_password": "new"},
                  headers=h).status_code == 200
    c.post("/api/logout")
    assert c.post("/api/login", json={"username": "admin", "password": "old"}).status_code == 401
    assert c.post("/api/login", json={"username": "admin", "password": "new"}).status_code == 200
...
def test_backup_and_restore_api(settings, stub_xray):
    ...
    assert doc["schema_version"] == 1 and any(n["name"] == "bk" for n in doc["nodes"])
    ...
    bad = {"schema_version": 2, "nodes": [], "subscriptions": [], "profiles": [],
